...
from tadm_polars.data import get_data_module
from tadm_polars.plotter import get_liquid_class_names, get_data_for_liquid_class, plot_both_steps
...
# def select_list_item(lst: list[str] | set[str]) -> str:
#     menu = sorted(x for x in lst)
#
#     for i, v in enumerate(menu):
#         rprint(f"{i + 1}) {v}")
#     i = IntPrompt.ask("Select a liquid class to plot (0 to exit)",
#                       choices=[str(x) for x in range(0, len(lst) + 1)],
#                       show_choices=False
#                       )
#     print("")
#     if i == 0:
